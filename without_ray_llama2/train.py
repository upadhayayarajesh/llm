...
class CustomLightningModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i, (start, end) in enumerate(zip(start_indices, end_indices)):
            ...
            if start <= end:  # ensure valid index ordering
                ...
                pred_answer = self.tokenizer.decode(
                    pred_answer_toekns, skip_special_tokens=True
                )
            else:
                pred_answer = ""
            predictions.append(pred_answer)
            true_answer = self.tokenizer.decode(
                input_ids[batch["start_positions"][i] : batch["end_positions"][i]],
        ...
        # compute F1 score
        f1_scores = [
            compute_f1_score(pred, true) for pred, true in zip(predictions, answers)
        ]
